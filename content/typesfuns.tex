...
\noindent
From the signature, we can see that this is a data type that takes a \tTC{Nat}, and produces a type.
...
Effectively, it is a type that captures integers that fall into the range of zero to \texttt{(n - 1)} where \tTC{n} is the argument used to instantiate the \tTC{Fin} type. 
For example, \texttt{Fin 5} can be thought of as the type of integers between 0 and 4.
...
As mentioned above, a useful application of the \tTC{Fin} family is to represent bounded natural numbers.
Since the first \tTC{n} natural numbers form a finite set of \tTC{n} elements, we can treat \tTC{Fin n} as the set of integers greater than or equal to zero and less than \tTC{n}.
...
The type checker guarantees that the location is no larger than the length of the vector, and of course no smaller than zero.

Note also that there is no case for \texttt{Nil} here.
This is because it is impossible. 
Since there is no element of \texttt{Fin Z}, and the location is a \texttt{Fin n}, then \texttt{n} can not be \tDC{Z}.
